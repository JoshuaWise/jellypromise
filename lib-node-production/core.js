...
function handle(self, deferred) {
	...
	if (self._66 === 0) {
		if (self._38 === 0) {
			...
			self._38 = 2
			self._6 = [self._6, deferred]
		} else {
			self._6.push(deferred)
		}
	} else {
...
function handleResolved(self, deferred) {
	asap(function () {
		...
		if (cb === null) {
			if (self._66 === 1) {
				resolve(deferred.promise, self._29)
			} else {
				reject(deferred.promise, self._29)
			}
		} else {
			var ret = tryCallOne(cb, self._29)
			if (ret === IS_ERROR) {
