...
Promise.prototype.then = function (onFulfilled, onRejected) {
	return this._41(onFulfilled, onRejected)
}
Promise.prototype.catch = function (onRejected) {
	if (arguments.length > 1) {
		...
		return this._41(null, function (reason) {
			for (var i=0; i<len; i++) {
	...
	return this._41(null, onRejected)
}
...
Promise.race = function (iterable) {
	return new Promise(INTERNAL)._64(function (res, rej) {
		...
		for (var i=0, len=input.length; i<len; i++) {
			Promise.resolve(input[i])._41(res, rej)
		}
	})
}
Promise.all = function (iterable) {
	return new Promise(INTERNAL)._64(function (res, rej) {
		...
		for (var i=0; i<pendings; i++) {
			Promise.resolve(input[i])._41(resolveItem(i), rej)
		}
	})
}
