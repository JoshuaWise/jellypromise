...
Promise.prototype.finally = function (fn) {
	...
	return this._40(function (value) {
		return Promise.resolve(fn())._40(function () {
			return value
		})
	}, function (reason) {
		return Promise.resolve(fn())._40(function () {
...
Promise.prototype.tap = function (fn) {
	...
	return this._40(function (value) {
		return Promise.resolve(fn(value))._40(function () {
			return value
		})
	})
}
Promise.prototype.else = function (value) {
	...
	return this._40(null, function () {return value})
}
...
Promise.prototype.timeout = function (ms, reason) {
	var self = this
	return new Promise(INTERNAL)._62(function (res, rej) {
...
Promise.any = function (iterable) {
	return new Promise(INTERNAL)._62(function (res, rej) {
		...
		for (var i=0; i<pendings; i++) {
			Promise.resolve(input[i])._40(res, fail)
		}
	})
}
Promise.props = function (obj) {
	return new Promise(INTERNAL)._62(function (res, rej) {
		...
		for (var i=0; i<pendings; i++) {
			var key = keys[i]
			Promise.resolve(obj[key])._40(resolveItem(key), rej)
		}
	})
}
Promise.settle = function (iterable) {
	return new Promise(INTERNAL)._62(function (res, rej) {
		...
		for (var i=0; i<pendings; i++) {
			...
			promise._40(handler, handler)
		}
	})
}
Promise.iterate = function (iterable, fn) {
	var promise = new Promise(INTERNAL)
	return promise._62(function (res, rej) {
		if (typeof fn !== 'function') {
		...
		;(function next() {
			...
			if (item.done) {
				...
				var p = Promise.resolve(item.value)._40(fn)._40(next)
				p._40(null, rej)
			}
		}())
	})
...
var PromiseDescriptor = function Promise(promise) {
	...
	if (promise._11 & 1) {
		...
		this.value = promise._77
	} else if (promise._11 & 2) {
		this.state = 'rejected'
		this.reason = promise._77
	} else {
		this.state = 'pending'
	}
}
