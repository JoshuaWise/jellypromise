...
Promise.prototype.tap = function (fn) {
	...
	return this._65(function (value) {
		return Promise.resolve(fn(value))._65(function () {
			return value
		})
	})
}
	...
	return promise._46(function (res, rej) {
		...
		;(function next() {
			...
			if (item.done) {
				...
				var p = Promise.resolve(item.value)._65(fn)._65(next)
				p._65(null, rej)
			}
		}())
	})
...
var PromiseDescriptor = function Promise(promise) {
	...
	if (promise._49 & 1) {
		...
		this.value = promise._47
	} else if (promise._49 & 2) {
		this.state = 'rejected'
		this.reason = promise._47
	} else {
		this.state = 'pending'
	}
}
