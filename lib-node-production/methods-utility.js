...
Promise.prototype.finally = function (fn) {
	...
	return this._41(function (value) {
		return Promise.resolve(fn())._41(function () {
			return value
		})
	}, function (reason) {
		return Promise.resolve(fn())._41(function () {
...
Promise.prototype.tap = function (fn) {
	...
	return this._41(function (value) {
		return Promise.resolve(fn(value))._41(function () {
			return value
		})
	})
}
Promise.prototype.else = function (value) {
	...
	return this._41(null, function () {return value})
}
...
Promise.any = function (iterable) {
	return new Promise(INTERNAL)._64(function (res, rej) {
		...
		for (var i=0; i<pendings; i++) {
			Promise.resolve(input[i])._41(res, fail)
		}
	})
}
Promise.props = function (obj) {
	return new Promise(INTERNAL)._64(function (res, rej) {
		...
		for (var i=0; i<pendings; i++) {
			var key = keys[i]
			Promise.resolve(obj[key])._41(resolveItem(key), rej)
		}
	})
}
Promise.settle = function (iterable) {
	return new Promise(INTERNAL)._64(function (res, rej) {
		...
		for (var i=0; i<pendings; i++) {
			...
			promise._41(handler, handler)
		}
	})
}
Promise.iterate = function (iterable, fn) {
	var promise = new Promise(INTERNAL)
	return promise._64(function (res, rej) {
		if (typeof fn !== 'function') {
		...
		;(function next() {
			...
			if (item.done) {
				...
				var p = Promise.resolve(item.value)._41(fn)._41(next)
				p._41(null, rej)
			}
		}())
	})
...
var PromiseDescriptor = function Promise(promise) {
	...
	if (promise._18 & 1) {
		...
		this.value = promise._74
	} else if (promise._18 & 2) {
		this.state = 'rejected'
		this.reason = promise._74
	} else {
		this.state = 'pending'
	}
}
