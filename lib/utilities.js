...
Promise.prototype.timeout = function (ms, reason) {
	...
	var self = this
	return new self.constructor(function (res, rej) {
...
Promise.any = function (iterable) {
	...
	return new self(function (res, rej) {
		...
		arr.forEach(function (item, i) {
			self.resolve(item).then(res, fail)
		})
	})
}
...
Promise.join = function (a, b, handler) {
	...
	return new self(function (res, rej) {
		...
		self.resolve(a).then(done).catch(rej)
		self.resolve(b).then(done).catch(rej)
	})
}
Promise.partition = function (iterable) {
	...
	return new self(function (res, rej) {
		...
		for (var i=0; i<pendings; i++) {
			self.resolve(arr[i]).then(pushResolved, pushRejected)
		}
