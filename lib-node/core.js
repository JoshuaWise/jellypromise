...
function handle(self, deferred) {
	...
	if (self._82 === 0) {
		if (self._33 === 0) {
			...
			self._33 = 2
			self._5 = [self._5, deferred]
		} else {
			self._5.push(deferred)
		}
	} else {
...
function handleResolved(self, deferred) {
	asap(function () {
		var cb = self._82 === 1 ? deferred.onFulfilled : deferred.onRejected
		if (cb === null) {
			if (self._82 === 1) {
				resolve(deferred.promise, self._32)
			} else {
				reject(deferred.promise, self._32)
			}
		} else {
			var ret = tryCallOne(cb, self._32)
			if (ret === IS_ERROR) {
