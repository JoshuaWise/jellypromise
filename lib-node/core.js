...
function finale(self) {
	if (self._14 === 1) {
		handle(self, self._71)
		self._71 = null
	} else if (self._14 === 2) {
		var deferreds = self._71
...
function handle(self, deferred) {
	...
	if (self._86 === 0) {
		if (self._14 === 0) {
			...
			self._14 = 2
			self._71 = [self._71, deferred]
		} else {
			self._71.push(deferred)
		}
	} else {
...
function handleResolved(self, deferred) {
	asap(function () {
		var cb = self._86 === 1 ? deferred.onFulfilled : deferred.onRejected
		if (cb === null) {
			if (self._86 === 1) {
				resolve(deferred.promise, self._11)
			} else {
				reject(deferred.promise, self._11)
			}
		} else {
			var ret = tryCallOne(cb, self._11)
			if (ret === IS_ERROR) {
