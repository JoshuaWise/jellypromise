...
Promise.prototype.finally = function (fn) {
	...
	return this._69(function (value) {
		return Promise.resolve(fn())._69(function () {
			return value
		})
	}, function (reason) {
		return Promise.resolve(fn())._69(function () {
...
Promise.prototype.tap = function (fn) {
	...
	return this._69(function (value) {
		return Promise.resolve(fn(value))._69(function () {
			return value
		})
	})
}
Promise.prototype.else = function (value) {
	...
	return this._69(null, function () {return value})
}
...
Promise.any = function (iterable) {
	return new Promise(INTERNAL)._16(function (res, rej) {
		...
		for (var i=0; i<pendings; i++) {
			Promise.resolve(input[i])._69(res, fail)
		}
	})
}
Promise.props = function (obj) {
	return new Promise(INTERNAL)._16(function (res, rej) {
		...
		for (var i=0; i<pendings; i++) {
			var key = keys[i]
			Promise.resolve(obj[key])._69(resolveItem(key), rej)
		}
	})
}
Promise.settle = function (iterable) {
	return new Promise(INTERNAL)._16(function (res, rej) {
		...
		for (var i=0; i<pendings; i++) {
			...
			promise._69(handler, handler)
		}
	})
}
Promise.iterate = function (iterable, fn) {
	var promise = new Promise(INTERNAL)
	return promise._16(function (res, rej) {
		if (typeof fn !== 'function') {
		...
		;(function next() {
			...
			if (item.done) {
				...
				var p = Promise.resolve(item.value)._69(fn)._69(next)
				p._94.parent = promise._94 // @[/development]
				p._69(null, rej)
			}
		}())
	})
...
var PromiseDescriptor = function Promise(promise) {
	...
	if (promise._88 & 1) {
		...
		this.value = promise._63
	} else if (promise._88 & 2) {
		this.state = 'rejected'
		this.reason = promise._63
	} else {
		this.state = 'pending'
	}
}
