...
Promise.prototype.finally = function (fn) {
	...
	return this._25(function (value) {
		return Promise.resolve(fn())._25(function () {
			return value
		})
	}, function (reason) {
		return Promise.resolve(fn())._25(function () {
			LST.setRejectionStack(self._53()._15) // @[/development]
			throw reason
		})
	})
}
Promise.prototype.tap = function (fn) {
	...
	return this._25(function (value) {
		return Promise.resolve(fn(value))._25(function () {
			return value
		})
	})
}
Promise.prototype.else = function (value) {
	...
	return this._25(null, function () {return value})
}
...
Promise.any = function (iterable) {
	return new Promise(INTERNAL)._2(function (res, rej) {
		...
		for (var i=0; i<pendings; i++) {
			Promise.resolve(input[i])._25(res, fail)
		}
	})
}
Promise.props = function (obj) {
	return new Promise(INTERNAL)._2(function (res, rej) {
		...
		for (var i=0; i<pendings; i++) {
			var key = keys[i]
			Promise.resolve(obj[key])._25(resolveItem(key), rej)
		}
	})
}
Promise.settle = function (iterable) {
	return new Promise(INTERNAL)._2(function (res, rej) {
		...
		for (var i=0; i<pendings; i++) {
			...
			promise._25(handler, handler)
		}
	})
}
Promise.iterate = function (iterable, fn) {
	var promise = new Promise(INTERNAL)
	return promise._2(function (res, rej) {
		if (typeof fn !== 'function') {
		...
		;(function next() {
			...
			if (item.done) {
				...
				var p = Promise.resolve(item.value)._25(fn)._25(next)
				p._15.parent = promise._15 // @[/development]
				p._25(null, rej)
			}
		}())
	})
...
var PromiseDescriptor = function Promise(promise) {
	...
	if (promise._10 & 1) {
		...
		this.value = promise._12
	} else if (promise._10 & 2) {
		this.state = 'rejected'
		this.reason = promise._12
	} else {
		this.state = 'pending'
	}
}
