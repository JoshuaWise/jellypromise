...
function Promise(fn) {
	if (!(this instanceof Promise) || this._89 !== undefined) {
...
Promise.prototype.then = function (onFulfilled, onRejected) {
	return this._91(onFulfilled, onRejected)
}
Promise.prototype.catch = function (onRejected) {
	if (arguments.length > 1) {
		...
		var newPromise
		return newPromise = this._91(null, function (reason) {
			for (var i=0; i<len; i++) {
	...
	return this._91(null, onRejected)
}
...
Promise.race = function (iterable) {
	return new Promise(INTERNAL)._46(function (res, rej) {
		...
		for (var i=0, len=input.length; i<len; i++) {
			Promise.resolve(input[i])._91(res, rej)
		}
	})
}
Promise.all = function (iterable) {
	return new Promise(INTERNAL)._46(function (res, rej) {
		...
		for (var i=0; i<pendings; i++) {
			Promise.resolve(input[i])._91(resolveItem(i), rej)
		}
	})
}
