...
Promise.prototype.then = function (onFulfilled, onRejected) {
	return this._48(onFulfilled, onRejected)
}
Promise.prototype.catch = function (onRejected) {
	if (arguments.length > 1) {
		...
		var newPromise
		return newPromise = this._48(null, function (reason) {
			for (var i=0; i<len; i++) {
	...
	return this._48(null, onRejected)
}
...
Promise.race = function (iterable) {
	return new Promise(INTERNAL)._4(function (res, rej) {
		...
		for (var i=0, len=input.length; i<len; i++) {
			Promise.resolve(input[i])._48(res, rej)
		}
	})
}
Promise.all = function (iterable) {
	return new Promise(INTERNAL)._4(function (res, rej) {
		...
		for (var i=0; i<pendings; i++) {
			Promise.resolve(input[i])._48(resolveItem(i), rej)
		}
	})
}
