...
function finale(self) {
	if (self._79 === 1) {
		handle(self, self._21)
		self._21 = null
	} else if (self._79 === 2) {
		var deferreds = self._21
...
function handle(self, deferred) {
	...
	if (self._20 === 0) {
		if (self._79 === 0) {
			...
			self._79 = 2
			self._21 = [self._21, deferred]
		} else {
			self._21.push(deferred)
		}
	} else {
...
function handleResolved(self, deferred) {
	asap(function () {
		var cb = self._20 === 1 ? deferred.onFulfilled : deferred.onRejected
		if (cb === null) {
			if (self._20 === 1) {
				resolve(deferred.promise, self._71)
			} else {
				reject(deferred.promise, self._71)
			}
		} else {
			var ret = tryCallOne(cb, self._71)
			if (ret === IS_ERROR) {
