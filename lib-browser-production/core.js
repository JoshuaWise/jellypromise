...
function finale(self) {
	if (self._49 === 1) {
		handle(self, self._66)
		self._66 = null
	} else if (self._49 === 2) {
		var deferreds = self._66
...
function handle(self, deferred) {
	...
	if (self._87 === 0) {
		if (self._49 === 0) {
			...
			self._49 = 2
			self._66 = [self._66, deferred]
		} else {
			self._66.push(deferred)
		}
	} else {
...
function handleResolved(self, deferred) {
	asap(function () {
		var cb = self._87 === 1 ? deferred.onFulfilled : deferred.onRejected
		if (cb === null) {
			if (self._87 === 1) {
				resolve(deferred.promise, self._78)
			} else {
				reject(deferred.promise, self._78)
			}
		} else {
			var ret = tryCallOne(cb, self._78)
			if (ret === IS_ERROR) {
