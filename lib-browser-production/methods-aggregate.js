...
Promise.prototype.filter = function (fn, ctx) {
	return this._65(function (iterable) {
		if (typeof fn !== 'function') {
			throw new TypeError('Expected first argument to be a function.')
		}
		var array = asArrayCopy(iterable)
		return mapArray(array, fn, ctx)._65(function (bools) {
			var result = []
...
Promise.prototype.forEach = function (fn, ctx) {
	return this._65(function (iterable) {
		...
		var array = asArrayCopy(iterable)
		return mapArray(array, fn, ctx)._65(function () {
			return array
		})
	})
}
	...
	return this._65(function (iterable) {
		...
		return new Promise(INTERNAL)._88(function (res, rej) {
			...
			var handler = function (item) {
				...
				return Promise.resolve(fn(result, item, i++, len))._65(setResult)
			}
			var next = function () {
				i === len
					? res(result)
					: Promise.resolve(array[i])._65(handler)._65(null, rej)
			}
			next()
		})
	})
	...
	return new Promise(INTERNAL)._88(function (res, rej) {
		...
		var each = function (i) {
			return function (value) {
				return Promise.resolve(fn.call(ctx, value, i, len))._65(function (value) {
					result[i] = value
		...
		for (var i=0, len=pendings; i<len; i++) {
			Promise.resolve(input[i])._65(each(i))._65(null, rej)
		}
	})
}
