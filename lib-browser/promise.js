...
function Promise(fn) {
	if (!(this instanceof Promise) || this._19 !== undefined) {
...
Promise.prototype.then = function (onFulfilled, onRejected) {
	return this._97(onFulfilled, onRejected)
}
Promise.prototype.catch = function (onRejected) {
	if (arguments.length > 1) {
		...
		var newPromise
		return newPromise = this._97(null, function (reason) {
			for (var i=0; i<len; i++) {
	...
	return this._97(null, onRejected)
}
...
Promise.race = function (iterable) {
	return new Promise(INTERNAL)._70(function (res, rej) {
		...
		for (var i=0, len=input.length; i<len; i++) {
			Promise.resolve(input[i])._97(res, rej)
		}
	})
}
Promise.all = function (iterable) {
	return new Promise(INTERNAL)._70(function (res, rej) {
		...
		for (var i=0; i<pendings; i++) {
			Promise.resolve(input[i])._97(resolveItem(i), rej)
		}
	})
}
