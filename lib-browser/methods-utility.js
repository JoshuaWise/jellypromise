...
Promise.prototype.finally = function (fn) {
	...
	return this._97(function (value) {
		return Promise.resolve(fn())._97(function () {
			return value
		})
	}, function (reason) {
		return Promise.resolve(fn())._97(function () {
			LST.setRejectionStack(self._94()._87) // @[/development]
			throw reason
		})
	})
}
Promise.prototype.tap = function (fn) {
	...
	return this._97(function (value) {
		return Promise.resolve(fn(value))._97(function () {
			return value
		})
	})
}
Promise.prototype.else = function (value) {
	...
	return this._97(null, function () {return value})
}
...
Promise.prototype.timeout = function (ms, reason) {
	var self = this
	return new Promise(INTERNAL)._70(function (res, rej) {
...
Promise.any = function (iterable) {
	return new Promise(INTERNAL)._70(function (res, rej) {
		...
		for (var i=0; i<pendings; i++) {
			Promise.resolve(input[i])._97(res, fail)
		}
	})
}
Promise.props = function (obj) {
	return new Promise(INTERNAL)._70(function (res, rej) {
		...
		for (var i=0; i<pendings; i++) {
			var key = keys[i]
			Promise.resolve(obj[key])._97(resolveItem(key), rej)
		}
	})
}
Promise.settle = function (iterable) {
	return new Promise(INTERNAL)._70(function (res, rej) {
		...
		for (var i=0; i<pendings; i++) {
			...
			promise._97(handler, handler)
		}
	})
}
Promise.iterate = function (iterable, fn) {
	var promise = new Promise(INTERNAL)
	return promise._70(function (res, rej) {
		if (typeof fn !== 'function') {
		...
		;(function next() {
			...
			if (item.done) {
				...
				var p = Promise.resolve(item.value)._97(fn)._97(next)
...
var PromiseDescriptor = function Promise(promise) {
	...
	if (promise._19 & 1) {
		...
		this.value = promise._54
	} else if (promise._19 & 2) {
		this.state = 'rejected'
		this.reason = promise._54
	} else {
		this.state = 'pending'
	}
}
