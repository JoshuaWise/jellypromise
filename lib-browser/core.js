...
function handleResolved(self, deferred) {
	asap(function () {
		var cb = self._28 === 1 ? deferred.onFulfilled : deferred.onRejected
		if (cb === null) {
			if (self._28 === 1) {
				resolve(deferred.promise, self._69)
			} else {
				reject(deferred.promise, self._69)
			}
		} else {
			var ret = tryCallOne(cb, self._69)
			if (ret === IS_ERROR) {
